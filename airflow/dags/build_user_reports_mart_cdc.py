...
DAG_ID = "build_user_reports_mart_cdc"
...
def extract_telemetry(**context):
    ...
    try:
        ...
        conn.close()
    context["ti"].xcom_push(key="telemetry_json", value=df.to_json(orient="records", date_format="iso"))
...
def load_telemetry_stage(**context):
    ...
    if not df.empty:
        ...
        rows = [tuple(row) for row in df[[
            "report_date","user_id","prosthesis_id","calibration_state","daily_movements",
            "avg_load","battery_cycles","avg_battery_level","alerts_count","telemetry_events_count"
...
def refresh_reporting_mart():
    client = ch()
    client.execute("TRUNCATE TABLE report_mart.user_reports_v2")
    client.execute(
        '''
        INSERT INTO report_mart.user_reports_v2
        SELECT
            t.report_date,
            t.user_id,
            c.crm_customer_id,
        ...
        FROM report_mart.telemetry_stage t
             ...
             (
                 SELECT
                     user_id,
                     anyLast(crm_customer_id) AS crm_customer_id,
                     anyLast(full_name) AS full_name,
                     anyLast(email) AS email,
                     anyLast(prosthesis_model) AS prosthesis_model
                 FROM report_mart.crm_profiles_current
                 WHERE deleted = 0 AND consent_granted = 1
                 GROUP BY user_id
             ) c
...
with DAG(
        ...
        schedule="0 * * * *",
        catchup=False,
        tags=["bionicpro", "reports", "cdc"],
) as dag:
    t1 = PythonOperator(task_id="extract_telemetry", python_callable=extract_telemetry)
    t2 = PythonOperator(task_id="load_telemetry_stage", python_callable=load_telemetry_stage)
    t3 = PythonOperator(task_id="refresh_reporting_mart", python_callable=refresh_reporting_mart)
